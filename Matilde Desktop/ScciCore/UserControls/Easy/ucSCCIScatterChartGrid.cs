...
{
    internal partial class ucSCCIScatterChartGrid : UserControl, Interfacce.IViewUserControlBase
    {
        #region DECLARE
        ...
        public void CaricaDati(ref DataTable dtGriglia, ref DataTable dtGrafico, float inizioscala, float finescala, DateTime datamin, DateTime datamax, string campodatapergrafico)
        {
            ...
            try
            {
                ...
                if (dtGrafico != null && dtGrafico.Rows.Count <= 0)
                {
                    ...
                    { 
                                                if (datamax > datamin)
                            datarif = DateTime.FromBinary(datamin.ToBinary() + ((datamax.ToBinary() - datamin.ToBinary()) / 2));
                        else
                            datarif = datamax;
                    }
                    ...
                    {
                        if (col.ColumnName == _campodatapergrafico)
                ...
                if (inizioscala == float.MinValue && finescala == float.MinValue && campodatapergrafico != "" && dtGrafico.Rows.Count > 0)
                {
                                                                ...
                                                                foreach (DataColumn ocol in dtGrafico.Columns)
                {
                    if (ocol.DataType.Name.ToUpper() != "DateTime".ToUpper())
                    {
                        try
                        {
                            ...
                            ntsS.DataBind(dtGrafico, campodatapergrafico, ocol.ColumnName);
        ...
        private void InizializzaGrafico()
        {
            try
            {
                ...
                this.ultraChart.Legend.Visible = true;
                                this.ultraChart.ScatterChart.ChartText[0].ChartTextFont = new System.Drawing.Font(this.Font.Name, easyStatics.getFontSizeInPointsFromRelativeDimension(easyStatics.easyRelativeDimensions.XSmall) - 1);
                this.ultraChart.ScatterChart.ChartText[0].ClipText = false;
                this.ultraChart.ScatterChart.ChartText[0].Column = -2;
                this.ultraChart.ScatterChart.ChartText[0].ItemFormatString = "<DATA_VALUE_Y:00.00#>";
                ...
                this.ultraChart.ScatterChart.LineAppearance.SplineTension = 0.27F;
