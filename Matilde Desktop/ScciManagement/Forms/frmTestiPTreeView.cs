...
{
    public partial class frmTestiPTreeView : Form, Interfacce.IViewFormBase
    {
        public frmTestiPTreeView()
        {
            InitializeComponent();
        }
        ...
        public void ViewInit()
        {
            ...
            this.picView.Image = Risorse.GetImageFromResource(MyStatics.GetNameResource(MyStatics.GC_TESTIPREDEFINITI, Enums.EnumImageSize.isz256));
        ...
        private void ActionGridToolClick(ToolBase oTool)
        {
            ...
            if (this.UltraTree.ActiveNode != null)
            {
                ...
                else if (this.UltraTree.ActiveNode.Tag.ToString() == MyStatics.TV_TESTOPREDEFINITO)
                {
                    string[] arrKey = this.UltraTree.ActiveNode.Key.Split('\\');
                    sCodTestoP = arrKey[arrKey.Length - 1];
            ...
            switch (oTool.Key)
            {
                case MyStatics.GC_NUOVO:
                    if (MyStatics.ActionDataNameFormPU(Enums.EnumDataNames.T_TestiPredefiniti, Enums.EnumModalityPopUp.mpNuovo, this.ViewIcon, this.picView.Image, this.ViewText, sCodTestoP, sPath) == DialogResult.OK)
                    {
                        this.LoadUltraTreePath();
                        this.LoadUltraTree();
                    }
                ...
                case MyStatics.GC_MODIFICA:
                    if (MyStatics.ActionDataNameFormPU(Enums.EnumDataNames.T_TestiPredefiniti, Enums.EnumModalityPopUp.mpModifica, this.ViewIcon, this.picView.Image, this.ViewText, sCodTestoP) == DialogResult.OK)
                    {
                        this.LoadUltraTreePath();
                        this.LoadUltraTree();
                    }
                ...
                case MyStatics.GC_ELIMINA:
                    if (MyStatics.ActionDataNameFormPU(Enums.EnumDataNames.T_TestiPredefiniti, Enums.EnumModalityPopUp.mpCancella, this.ViewIcon, this.picView.Image, this.ViewText, sCodTestoP) == DialogResult.OK)
                    {
                        this.LoadUltraTreePath();
                        this.LoadUltraTree();
                    }
        ...
        private void LoadUltraTreePath()
        {
            ...
            try
            {
                ...
                string sSql = @"Select IsNull(Path,'') As Path From T_TestiPredefiniti Where IsNull(Path, '') <> '' Group By Path Order By Path";
        ...
        private void LoadUltraTree()
        {
            ...
            try
            {
                ...
                UltraTreeNode oNode = null;
                UltraTreeNode oNodeParent = null;
                string sSql = @"Select * From T_TestiPredefiniti";
                DataSet oDs = DataBase.GetDataSet(sSql);
                foreach (DataRow oDataRow in oDs.Tables[0].Rows)
                {
                    ...
                    if (oDataRow["Path"].ToString().Trim() == "")
                    {
                        ...
                        if (oNodeParent != null)
                        {
                            ...
                            oNode.LeftImages.Add(Risorse.GetImageFromResource(MyStatics.GetNameResource(MyStatics.GC_TESTIPREDEFINITI, Enums.EnumImageSize.isz16)));
                            oNode.Tag = MyStatics.TV_TESTOPREDEFINITO;
        ...
        private void UltraTree_DragDrop(object sender, DragEventArgs e)
        {
            ...
            try
            {
                ...
                if (oUltraTreeNode != null && oItem != null)
                {
                    ...
                    string sSql = string.Format("Update T_TestiPredefiniti Set Path = '{0}' Where Codice = '{1}'", oItem.Key, MyStatics.g_split.GetValue(MyStatics.g_split.Length - 1));
        ...
        private void UltraTree_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (UltraTree.SelectedNodes[0].Tag.ToString() == MyStatics.TV_TESTOPREDEFINITO)
                    {
