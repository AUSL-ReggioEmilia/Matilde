CREATE PROCEDURE [dbo].[MSP_BO_SpostaGruppoTaskInfermieristici]
		(@xParametri XML, 
		 @bErrore BIT OUTPUT		
		 )  
AS
BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GWKIO001) ERRORE : nessuna cartella di origine trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN						
	...
	BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GWKIO002) ERRORE : nessuna cartella di destinazione trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN						
	...
	BEGIN
		SET @bErrore=1
		INSERT INTO @tblErrori
				VALUES ('(GWKIO003) IDPaziente NON specificato')
	END
	ELSE
	BEGIN								
				SET @nRecord=(
					SELECT COUNT(*) FROM T_MovTaskInfermieristici
					  WHERE 
		...
		BEGIN
			...
			INSERT INTO @tblErrori
				VALUES ('(GWKIO004) ERRORE Nessun Task Infermieristico da aggiornare per la cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')
		END
		ELSE
		BEGIN	
			...
			BEGIN
								SET @bErrore=1
					INSERT INTO @tblErrori
						VALUES('(GWKIO005) ERRORE Stai cercando di spostare WKI sulla stessa cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')	
			END
			...
			BEGIN
				...
				BEGIN TRANSACTION											
					...
					BEGIN								
						...
						SET @xTimeStampSingolo.modify('insert <IDEntitaInizio>{sql:variable("@uIDTaskInfermieristico")}</IDEntitaInizio> into (/)[1]')
						...
						SET @xTimeStampSingolo=CONVERT(XML,
										...
										'</Parametri>')						
						 						...
						 						INSERT INTO @tblErrori 
							EXEC MSP_BO_SpostaTaskInfermieristici @xTimeStampSingolo, @bErroreSingolo  OUTPUT
