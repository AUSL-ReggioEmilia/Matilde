CREATE PROCEDURE [dbo].[MSP_WS_CercaPrescrizioniTempi](@xParametri AS XML)
AS
BEGIN
	...
	IF 	ISNULL(@sGUID,'') <> '' SET @uIDPrescrizione=CONVERT(UNIQUEIDENTIFIER,	@sGUID)	
	...
	SET @sCodTipoPrescrizioneTempi= LTRIM(RTRIM(ISNULL(@sCodTipoPrescrizioneTempi,'')))	
	...
	SET @sCodLogin= LTRIM(RTRIM(ISNULL(@sCodLogin,'')))
		...
		IF @sCodTipoPrescrizioneTempi NOT IN ('')
		BEGIN							 				
			SET @sWhere= @sWhere +  ' AND M.CodTipoPrescrizioneTempi ='''+ @sCodTipoPrescrizioneTempi + '''' 													
		END
	...
	SET @sSQL='	SELECT
						...
						M.IDPrescrizione AS IDPrescrizione,
						M.DataEvento,												
	...
	SET @sSQL=@sSQL + '
				FROM
						...
						LEFT JOIN Q_SelPrescrizioniTempiTask PRETMPTASK							
							ON PRETMPTASK.IDPrescrizioneTempi = CONVERT(VARCHAR(50),M.ID)
	...
	SET @sWhere= @sWhere + ' AND M.CodStatoPrescrizioneTempi NOT IN (''CA'')
						     AND M.IDPrescrizione =''' + CONVERT(VARCHAR(50),@uIDPrescrizione) + '''
							'
		IF ISNULL(@sWhere,'')<> ''
	BEGIN
