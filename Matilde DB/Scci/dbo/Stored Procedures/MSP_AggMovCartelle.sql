CREATE PROCEDURE [dbo].[MSP_AggMovCartelle](@xParametri XML)
AS
BEGIN
	...
	DECLARE @sMotivoRiapertura AS VARCHAR(500)
	...
	DECLARE @sCodStatoCartellaMemorizzato AS VARCHAR(20)
	...
	ELSE
		...
		SET @sSQL='UPDATE T_MovCartelle ' + CHAR(13) + CHAR(10) +
			  'SET '
	...
	SET @sCodStatoCartellaMemorizzato=(SELECT TOP 1 CodStatoCartella						  
									   FROM T_MovCartelle
									   WHERE ID=@uIDCartella)

	SET @sCodStatoCartellaMemorizzato=ISNULL(@sCodStatoCartellaMemorizzato,'')
	...
	END	
			...
			IF @xParametri.exist('/Parametri/MotivoRiapertura')=1		
		BEGIN			
			SET @sMotivoRiapertura = (SELECT TOP 1 ValoreParametro.MotivoRiapertura.value('.','VARCHAR(500)')
										FROM @xParametri.nodes('/Parametri/MotivoRiapertura') as ValoreParametro(MotivoRiapertura))		

			IF ISNULL(@sMotivoRiapertura,'') = ''
				SET @sSET=@sSET + ',MotivoRiapertura=NULL'	
			ELSE
				SET @sSET=@sSET + ',MotivoRiapertura=''' + @sMotivoRiapertura + ''''
		END
		ELSE
		BEGIN
			SET @sMotivoRiapertura=NULL	
		END

		IF @xParametri.exist('/Parametri/CodStatoCartellaInfo')=1
	BEGIN			
		...
		IF ISNULL(@sCodStatoCartellaInfo,'') <> ''
			SET @sSET=@sSET + ',CodStatoCartellaInfo=''' + @sCodStatoCartellaInfo + ''''
		ELSE			
			SET @sSET=@sSET + ',CodStatoCartellaInfo=NULL'
	END
	ELSE
	BEGIN
	...
	IF @sSET <> ''	
		BEGIN
			...
			SET @xTimeStamp.modify('insert <CodEntita>CAR</CodEntita> into (/TimeStamp)[1]')
						...
						IF @sCodStatoCartellaMemorizzato='CH' AND @sCodAzione='MOD' AND ISNULL(@txtPDFCartella,'') <> ''
			BEGIN				
