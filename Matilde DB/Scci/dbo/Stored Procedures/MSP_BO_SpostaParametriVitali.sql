CREATE PROCEDURE [dbo].[MSP_BO_SpostaParametriVitali](@xParametri XML, @bErrore BIT OUTPUT)  
AS
BEGIN
		...
		ELSE
			BEGIN				
				...
				SET @uIDPazienteInizio=(SELECT IDPaziente FROM T_MovPazienti WHERE IDEpisodio=@uIDEpisodioInizio)
								...
								SET @uIDTrasferimentoInizio=(SELECT IDTrasferimento FROM T_MovParametriVitali WHERE ID=@uIDEntitaInizio)												
	...
	BEGIN
			...
			BEGIN
				SET @bErrore=1
				INSERT INTO #tmpErrori(Errore)
					VALUES('(PVT001) IDParametro Vitale NON specificato')
			
			END
		ELSE
		BEGIN					
						SET @nRecord=(SELECT COUNT(*) FROM T_MovParametriVitali 
				...
				BEGIN
										SET @bErrore=1
					INSERT INTO #tmpErrori(Errore)
							VALUES('(PVT002) ERRORE Nessun Parametro Vitale da aggiornare {' + CONVERT(VARCHAR(50),@uIDEntitaInizio)+ '}')	
				END
			ELSE
			BEGIN	
						...
						BEGIN
							...
							ELSE
								BEGIN							
									...
									SET @bErrore=1
									INSERT INTO #tmpErrori(Errore)
										VALUES('(PVT003) ERRORE durante aggiornamento della scheda del PVT {' + CONVERT(VARCHAR(50),@uIDEntitaInizio)+ '}')
								END	
						END		  
					ELSE
						BEGIN					
							...
							SET @bErrore=1
							INSERT INTO #tmpErrori(Errore)
										VALUES('(PVT004) ERRORE durante aggiornamento del PVT')
						END	
				...
				BEGIN			
					...
					SET @xTimeStamp.modify('insert <IDTrasferimento>{sql:variable("@uIDTrasferimentoInizio")}</IDTrasferimento> into (/TimeStamp)[1]')
										...
										SET @sInfoTimeStamp='Azione: BO_SpostaPVT'
