CREATE PROCEDURE [dbo].[MSP_AggMovAlertGenerici](@xParametri XML)
AS
BEGIN
	...
	DECLARE @xParLog AS XML
		...
		SET @sSQL='UPDATE T_MovAlertGenerici ' + CHAR(13) + CHAR(10) +
			  'SET '  				  						
			  					
	SET @sSET =''
	
		IF @xParametri.exist('/Parametri/IDAlertGenerico')=1
		BEGIN
			...
			IF 	ISNULL(@sGUID,'') <> '' 
					SET @uIDAlertGenerico=CONVERT(UNIQUEIDENTIFIER,	@sGUID)	
					END
	...
	END	
		...
		IF @xParametri.exist('/Parametri/CodTipoAlertGenerico')=1
	BEGIN
		...
		IF @sCodTipoAlertGenerico <> ''
				SET	@sSET= @sSET + ',CodTipoAlertGenerico=''' + @sCodTipoAlertGenerico +''''	+ CHAR(13) + CHAR(10)				
			ELSE
				SET	@sSET= @sSET + ',CodTipoAlertGenerico=NULL'	+ CHAR(13) + CHAR(10)		
	END	


		IF @xParametri.exist('/Parametri/CodStatoAlertGenerico')=1
	BEGIN
		...
		IF @sCodStatoAlertGenerico <> ''
				SET	@sSET= @sSET + ',CodStatoAlertGenerico=''' + @sCodStatoAlertGenerico +''''	+ CHAR(13) + CHAR(10)				
			ELSE
				SET	@sSET= @sSET + ',CodStatoAlertGenerico=NULL'	+ CHAR(13) + CHAR(10)		
	END	
	...
	ELSE
		...
		SET	@sSET= @sSET + ',DataUltimaModificaUTC=GetUTCDate()'	+ CHAR(13) + CHAR(10)					
					...
					SET @sCodAzione=(SELECT TOP 1 ValoreParametro.CodAzione.value('.','VARCHAR(100)')
					  FROM @xParametri.nodes('/Parametri/TimeStamp/CodAzione') as ValoreParametro(CodAzione))	
	SET @sCodAzione=ISNULL(@sCodAzione,'')
	
	IF @sCodAzione='VIS'		
	BEGIN
			SET	@sSET= @sSET + ',CodUtenteVisto=  '''	+ @sCodUtenteUltimaModifica + '''' + CHAR(13) + CHAR(10)	
			SET	@sSET= @sSET + ',DataVisto=  GetDate()'
			SET	@sSET= @sSET + ',DataVistoUTC=  GetUTCDate()'
	END
	...
	IF @sSET <> ''		
		BEGIN
								...
								SET @sWHERE =' WHERE ID=''' + convert(varchar(50),@uIDAlertGenerico) +''''
				...
				SET @xTimeStamp.modify('insert <IDEntita>{sql:variable("@uIDAlertGenerico")}</IDEntita> into (/TimeStamp)[1]')
				...
				SET @xTemp=
					(SELECT * FROM 
						(SELECT * FROM T_MovAlertGenerici
						 WHERE ID=@uIDAlertGenerico											) AS [Table]
