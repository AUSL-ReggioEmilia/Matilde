CREATE PROCEDURE [dbo].[MSP_BO_SpostaTaskInfermieristici](@xParametri XML, @bErrore BIT OUTPUT)  
AS
BEGIN
		...
		ELSE
			BEGIN				
				...
				SET @uIDPazienteInizio=(SELECT IDPaziente FROM T_MovPazienti WHERE IDEpisodio=@uIDEpisodioInizio)
								...
								SET @uIDTrasferimentoInizio=(SELECT IDTrasferimento FROM T_MovTaskInfermieristici WHERE ID=@uIDEntitaInizio)												
	...
	BEGIN
		...
		BEGIN
			SET @bErrore=1
			INSERT INTO #tmpErrori(Errore)
				VALUES('(WKI001) IDTaskInfermieristico NON specificato')
		
		END
		ELSE
		BEGIN					
						SET @nRecord=(SELECT COUNT(*) FROM T_MovTaskInfermieristici
			...
			BEGIN
								SET @bErrore=1
				INSERT INTO #tmpErrori(Errore)
						VALUES('(WKI002) ERRORE Nessun Task Infermieristico da aggiornare {' + CONVERT(VARCHAR(50),@uIDEntitaInizio)+ '}')
			END
			ELSE
			BEGIN	
						...
						BEGIN
							...
							ELSE
								BEGIN							
									...
									SET @bErrore=1
									INSERT INTO #tmpErrori(Errore)
										VALUES('(WKI003) ERRORE durante aggiornamento della scheda del WKI {' + CONVERT(VARCHAR(50),@uIDEntitaInizio)+ '}')
								END	
						END		  
					ELSE
						BEGIN					
							...
							SET @bErrore=1
							INSERT INTO #tmpErrori(Errore)
										VALUES('(WKI004) ERRORE durante aggiornamento del WKI {' + CONVERT(VARCHAR(50),@uIDEntitaInizio)+ '}')
						END	
				...
				BEGIN			
					...
					SET @xTimeStamp.modify('insert <IDTrasferimento>{sql:variable("@uIDTrasferimentoInizio")}</IDTrasferimento> into (/TimeStamp)[1]')
										...
										SET @sInfoTimeStamp='Azione: BO_SpostaWKI'
