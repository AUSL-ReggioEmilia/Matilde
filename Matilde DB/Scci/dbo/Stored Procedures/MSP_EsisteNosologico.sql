CREATE PROCEDURE [dbo].[MSP_EsisteNosologico] (@xParametri XML)
AS
BEGIN
	...
	IF @sNumeroNosologico<>@sNumeroListaAttesa 
		BEGIN 
						SET @uIDEpisodio=
							...
							WHERE
								...
								AND 
								ISNULL(NumeroListaAttesa, '') =
									CASE
										WHEN @sNumeroListaAttesa = '.' THEN ISNULL(NumeroListaAttesa, '')											ELSE @sNumeroListaAttesa																				END
		...
		ELSE
			BEGIN
							...
							FROM T_MovEpisodi 
							WHERE
								(ISNULL(NumeroNosologico,'') = @sNumeroNosologico 
									OR
								ISNULL(NumeroListaAttesa,'') = @sNumeroListaAttesa
								)
