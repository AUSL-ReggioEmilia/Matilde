CREATE PROCEDURE [dbo].[MSP_BO_SpostaPrescrizioni](@xParametri XML, @bErrore BIT OUTPUT)  
AS
BEGIN
		...
		ELSE
			BEGIN				
				...
				SET @uIDPazienteInizio=(SELECT IDPaziente FROM T_MovPazienti WHERE IDEpisodio=@uIDEpisodioInizio)
								...
								SET @uIDTrasferimentoInizio=(SELECT IDTrasferimento FROM T_MovPrescrizioni WHERE ID=@uIDEntitaInizio)												
	...
	BEGIN
		...
		WHERE IDCartella 
			...
			AND T_MovTrasferimenti.DataUscita IS NULL 
	...
	END	
		...
		BEGIN
			SET @bErrore=1
			INSERT INTO #tmpErrori(Errore)
				VALUES('(PRF001) IDPrescrizione NON specificato')
	...
	BEGIN								
			...
			BEGIN
								SET @bErrore=1
				INSERT INTO #tmpErrori(Errore)
						VALUES('(PRF002) ERRORE Nessuna Prescrizione da aggiornare {' + CONVERT(VARCHAR(50),@uIDEntitaInizio) +'}')	
			END
		ELSE
		BEGIN	
					...
					BEGIN
						...
						ELSE
							BEGIN							
								...
								SET @bErrore=1
								INSERT INTO #tmpErrori(Errore)
									VALUES('(PRF003) ERRORE durante aggiornamento della scheda della PRF {' + CONVERT(VARCHAR(50),@uIDEntitaInizio) +'}')	
							END	
						...
						SET			
							...
							IDEpisodio=@uIDEpisodioFine,
							IDTrasferimento=@uIDTrasferimentoFine
						WHERE IDEntita IN (SELECT ID FROM T_MovPrescrizioniTempi WHERE IDPrescrizione=@uIDEntitaInizio) AND
							  CodEntita='PRT'
					END		  
				ELSE
					BEGIN					
						...
						SET @bErrore=1
						INSERT INTO #tmpErrori(Errore)
									VALUES('(PRF004) ERRORE durante aggiornamento della PRF {' + CONVERT(VARCHAR(50),@uIDEntitaInizio) +'}')
					END	
			...
			BEGIN			
				...
				SET @xTimeStamp.modify('insert <IDTrasferimento>{sql:variable("@uIDTrasferimentoInizio")}</IDTrasferimento> into (/TimeStamp)[1]')
								...
								SET @sInfoTimeStamp='Azione: BO_SpostaPRF'
			...
			END 			
				...
				SET 						
					IDEpisodio=@uIDEpisodioFine,
					IDTrasferimento=@uIDTrasferimentoFine
				WHERE IDSistema=CONVERT(VARCHAR(50),@uIDEntitaInizio)
					...
					BEGIN
						...
						ELSE
							BEGIN							
								...
								SET @bErrore=1
								INSERT INTO #tmpErrori(Errore)
									VALUES('(PRF005) ERRORE durante aggiornamento della scheda della WKI da PRF  {' + CONVERT(VARCHAR(50),@uIDEntitaInizio) +'}')
							END	
					END		  
				ELSE
					BEGIN					
						...
						SET @bErrore=1
						INSERT INTO #tmpErrori(Errore)
									VALUES('(PRF006) ERRORE durante aggiornamento del WKI da PRF  {' + CONVERT(VARCHAR(50),@uIDEntitaInizio) +'}')
					END	
		END	END
