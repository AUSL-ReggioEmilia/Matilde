CREATE PROCEDURE [dbo].[MSP_BO_SpostaAlertGenerico](@xParametri XML, @bErrore BIT OUTPUT)  
AS
BEGIN
		...
		ELSE
			BEGIN				
				...
				SET @uIDPazienteInizio=(SELECT IDPaziente FROM T_MovPazienti WHERE IDEpisodio=@uIDEpisodioInizio)
								...
								SET @uIDTrasferimentoInizio=(SELECT IDTrasferimento FROM T_MovAlertGenerici WHERE ID=@uIDEntitaInizio)												
	...
	BEGIN
			...
			BEGIN
				SET @bErrore=1
				INSERT INTO #tmpErrori(Errore)
					VALUES('(ALG001) IDWarning NON specificato')
			
			END
		ELSE
		BEGIN					
						SET @nRecord=(SELECT COUNT(*) FROM T_MovAlertGenerici
				...
				BEGIN
										SET @bErrore=1
					INSERT INTO #tmpErrori(Errore)
							VALUES('(ALG002) ERRORE Nessun Warning da aggiornare {' + CONVERT(VARCHAR(50),@uIDEntitaInizio)+ '}')	
				END
			ELSE
			BEGIN	
						...
						BEGIN
							...
							ELSE
								BEGIN							
									...
									SET @bErrore=1
									INSERT INTO #tmpErrori(Errore)
										VALUES('(ALG003) ERRORE durante aggiornamento della scheda del ALG {' + CONVERT(VARCHAR(50),@uIDEntitaInizio)+ '}')
								END	
						END		  
					ELSE
						BEGIN					
							...
							SET @bErrore=1
							INSERT INTO #tmpErrori(Errore)
										VALUES('(ALG004) ERRORE durante aggiornamento del ALG')
						END	
				...
				BEGIN			
					...
					SET @xTimeStamp.modify('insert <IDTrasferimento>{sql:variable("@uIDTrasferimentoInizio")}</IDTrasferimento> into (/TimeStamp)[1]')
										...
										SET @sInfoTimeStamp='Azione: MSP_BO_SpostaAlertGenerico'
					SET @sInfoTimeStamp=@sInfoTimeStamp+ '  IDCartellaInizio: ' + CONVERT(VARCHAR(50),@uIDCartellaInizio)  
					SET @sInfoTimeStamp=@sInfoTimeStamp+ '  IDPazienteInizio: ' + CONVERT(VARCHAR(50),@uIDPazienteInizio)  
					SET @sInfoTimeStamp=@sInfoTimeStamp+ '  IDEpisodioInizio: ' + CONVERT(VARCHAR(50),@uIDEpisodioInizio)  
					SET @sInfoTimeStamp=@sInfoTimeStamp + ' IDCartellaFine:' + ISNULL(CONVERT(VARCHAR(50),@uIDCartellaFine),'')
					SET @sInfoTimeStamp=@sInfoTimeStamp + ' IDPazienteFine: ' + ISNULL(CONVERT(VARCHAR(50),@uIDPazienteFine),'')
					SET @sInfoTimeStamp=@sInfoTimeStamp+ '  IDEpisodioFine: ' + ISNULL(CONVERT(VARCHAR(50),@uIDEpisodioFine),'')
