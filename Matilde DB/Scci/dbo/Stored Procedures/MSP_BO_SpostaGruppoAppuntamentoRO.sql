CREATE PROCEDURE [dbo].[MSP_BO_SpostaGruppoAppuntamentoRO]
		(@xParametri XML, 
		 @bErrore BIT OUTPUT		
		 )  
AS
BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GAPPRO001) ERRORE : nessuna cartella di origine trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN						
	...
	BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GAPPRO002) ERRORE : nessuna cartella di destinazione trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN						
	...
	BEGIN
		SET @bErrore=1
		INSERT INTO @tblErrori
				VALUES ('(GAPPRO003) IDPaziente NON specificato')
	END
	ELSE
	BEGIN								
				SET @nRecord=(SELECT COUNT(*) FROM T_MovAppuntamenti 		
					  WHERE IDPaziente=@uIDPazienteInizio AND										        IDEpisodio=@uIDEpisodioInizio AND										        IDTrasferimento																		IN (SELECT ID FROM T_MovTrasferimenti
									WHERE 
		...
		BEGIN
			...
			INSERT INTO @tblErrori
				VALUES ('(GAPPRO004) ERRORE Nessun Appuntamento per Ricoverati da aggiornare per la cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')
		END
		ELSE
		BEGIN	
			...
			BEGIN
								SET @bErrore=1
					INSERT INTO @tblErrori
						VALUES('(GAPPRO005) ERRORE Stai cercando di spostare appuntamenti sulla stessa cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')	
			END
			...
			BEGIN
				...
				WHERE
					IDPaziente=@uIDPazienteInizio AND										IDEpisodio=@uIDEpisodioInizio AND										IDTrasferimento																IN (SELECT ID FROM T_MovTrasferimenti
							WHERE 
								IDEpisodio=@uIDEpisodioInizio AND 
								IDCartella=@uIDCartellaInizio) 	
				...
				BEGIN TRANSACTION											
					...
					BEGIN								
						...
						INSERT INTO @tblErrori 
							EXEC MSP_BO_SpostaAppuntamentoRo @xTimeStampSingolo, @bErroreSingolo  OUTPUT
