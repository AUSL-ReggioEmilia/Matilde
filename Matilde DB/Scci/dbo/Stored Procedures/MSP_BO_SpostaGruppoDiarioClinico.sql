CREATE PROCEDURE [dbo].[MSP_BO_SpostaGruppoDiarioClinico]
		(@xParametri XML, 
		 @bErrore BIT OUTPUT		
		 )  
AS
BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GDCLO001) ERRORE : nessuna cartella di origine trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN						
	...
	BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GDCLO002) ERRORE : nessuna cartella di destinazione trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN						
	...
	BEGIN
		SET @bErrore=1
		INSERT INTO @tblErrori
				VALUES ('(GDCLO003) IDPaziente NON specificato')
	END
	ELSE
	BEGIN								
				SET @nRecord=(
		...
		BEGIN
			...
			INSERT INTO @tblErrori
				VALUES ('(GDCLO004) ERRORE Nessuna registrazione di DCL da aggiornare per la cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')
		END
		ELSE
		BEGIN	
			...
			BEGIN
								SET @bErrore=1
					INSERT INTO @tblErrori
						VALUES('(GAPPRO005) ERRORE Stai cercando di spostare DCL sulla stessa cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')	
			END
			...
			BEGIN
				...
				BEGIN TRANSACTION											
					...
					BEGIN								
						...
						SET @xTimeStampSingolo.modify('insert <IDEntitaInizio>{sql:variable("@uIDDiarioClinico")}</IDEntitaInizio> into (/)[1]')
						...
						INSERT INTO @tblErrori 
							EXEC MSP_BO_SpostaDiarioClinico @xTimeStampSingolo, @bErroreSingolo  OUTPUT
