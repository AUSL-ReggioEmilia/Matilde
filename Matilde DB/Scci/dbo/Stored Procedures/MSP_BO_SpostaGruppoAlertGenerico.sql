CREATE PROCEDURE [dbo].[MSP_BO_SpostaGruppoAlertGenerico]
		(@xParametri XML, 
		 @bErrore BIT OUTPUT		
		 )  
AS
BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GALGO001) ERRORE : nessuna cartella di origine trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN						
	...
	BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GALGO002) ERRORE : nessuna cartella di destinazione trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN						
	...
	BEGIN
		SET @bErrore=1
		INSERT INTO @tblErrori
				VALUES ('(GALGO003) IDPaziente NON specificato')
	END
	ELSE
	BEGIN								
				SET @nRecord=(
					SELECT COUNT(*) FROM T_MovAlertGenerici
					  WHERE 
		...
		BEGIN
			...
			INSERT INTO @tblErrori
				VALUES ('(GALGO004) ERRORE Nessun Warning da aggiornare per la cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')
		END
		ELSE
		BEGIN	
			...
			BEGIN
								SET @bErrore=1
					INSERT INTO @tblErrori
						VALUES('(GALGO005) ERRORE Stai cercando di spostare ALG sulla stessa cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')	
			END
			...
			BEGIN
				...
				BEGIN TRANSACTION											
					...
					BEGIN								
						...
						INSERT INTO @tblErrori 
							EXEC MSP_BO_SpostaAlertGenerico @xTimeStampSingolo, @bErroreSingolo  OUTPUT
