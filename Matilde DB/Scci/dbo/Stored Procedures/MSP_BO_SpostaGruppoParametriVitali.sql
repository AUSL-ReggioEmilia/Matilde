CREATE PROCEDURE [dbo].[MSP_BO_SpostaGruppoParametriVitali]
		(@xParametri XML, 
		 @bErrore BIT OUTPUT		
		 )  
AS
BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GPVTO001) ERRORE : nessuna cartella di origine trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN						
	...
	BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GPVTO002) ERRORE : nessuna cartella di destinazione trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN						
	...
	BEGIN
		SET @bErrore=1
		INSERT INTO @tblErrori
				VALUES ('(GPVTO003) IDPaziente NON specificato')
	END
	ELSE
	BEGIN								
				SET @nRecord=(
					SELECT COUNT(*) FROM T_MovParametriVitali 
					  WHERE 
		...
		BEGIN
			...
			INSERT INTO @tblErrori
				VALUES ('(GPVTO004) ERRORE Nessun Parametro Vitale da aggiornare per la cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')
		END
		ELSE
		BEGIN	
			...
			BEGIN
								SET @bErrore=1
					INSERT INTO @tblErrori
						VALUES('(GPVTO005) ERRORE Stai cercando di spostare PVT sulla stessa cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')	
			END
			...
			BEGIN
				...
				BEGIN TRANSACTION											
					...
					BEGIN								
						...
						SET @xTimeStampSingolo.modify('insert <IDEntitaInizio>{sql:variable("@uIDParametroVitale")}</IDEntitaInizio> into (/)[1]')
						...
						INSERT INTO @tblErrori 
							EXEC MSP_BO_SpostaParametriVitali @xTimeStampSingolo, @bErroreSingolo  OUTPUT
