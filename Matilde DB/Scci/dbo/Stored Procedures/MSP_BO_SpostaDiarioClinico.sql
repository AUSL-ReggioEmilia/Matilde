CREATE PROCEDURE [dbo].[MSP_BO_SpostaDiarioClinico](@xParametri XML, @bErrore BIT OUTPUT)  
AS
BEGIN
		...
		ELSE
			BEGIN				
				...
				SET @uIDPazienteInizio=(SELECT IDPaziente FROM T_MovPazienti WHERE IDEpisodio=@uIDEpisodioInizio)
								...
								SET @uIDTrasferimentoInizio=(SELECT IDTrasferimento FROM T_MovDiarioClinico WHERE ID=@uIDEntitaInizio)												
	...
	BEGIN
			...
			BEGIN
				SET @bErrore=1
				INSERT INTO #tmpErrori(Errore)
					VALUES('(DCL001) IDDiario Clinico NON specificato')
			
			END
		ELSE
		BEGIN					
						SET @nRecord=(SELECT COUNT(*) FROM T_MovDiarioClinico 
				...
				BEGIN
										SET @bErrore=1
					INSERT INTO #tmpErrori(Errore)
							VALUES('(DCL002) ERRORE Nessun Diario Clinico da aggiornare')	
				END
			ELSE
				BEGIN					
							...
							BEGIN
								...
								ELSE
									BEGIN							
										...
										SET @bErrore=1
										INSERT INTO #tmpErrori(Errore)
											VALUES('(DCL003) ERRORE durante aggiornamento della scheda del DCL {' + CONVERT(VARCHAR(50),@uIDEntitaInizio)+ '}')
									END	
							END		  
						ELSE
							BEGIN					
								...
								SET @bErrore=1
								INSERT INTO #tmpErrori(Errore)
											VALUES('(DCL004) ERRORE durante aggiornamento del DCL {' + CONVERT(VARCHAR(50),@uIDEntitaInizio)+ '}')
							END
					...
					BEGIN			
						...
						SET @xTimeStamp.modify('insert <IDTrasferimento>{sql:variable("@uIDTrasferimentoInizio")}</IDTrasferimento> into (/TimeStamp)[1]')
												...
												SET @sInfoTimeStamp='Azione: BO_SpostaDCL'
