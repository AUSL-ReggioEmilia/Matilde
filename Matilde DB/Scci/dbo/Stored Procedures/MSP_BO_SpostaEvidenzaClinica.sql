CREATE PROCEDURE [dbo].[MSP_BO_SpostaEvidenzaClinica](@xParametri XML, @bErrore BIT OUTPUT)  
AS
BEGIN
	...
	IF @uIDEntitaInizio IS NOT NULL AND @uIDEpisodioFine IS NOT NULL
	BEGIN
		
		SELECT  @nQta=COUNT(*)			
		FROM T_MovEvidenzaClinica
		WHERE ID=@uIDEntitaInizio AND IDEpisodio <> @uIDEpisodioFine
		...
		SELECT @nQta=COUNT(*)
		FROM T_MovEpisodi
		WHERE ID=@uIDEpisodioFine	
		
		IF @nQta=0
		BEGIN
			INSERT INTO #tmpErrori(Errore)
						VALUES('(GG002) ERRORE : nessuna episodio di destinazione trovata')
			SET @bErrore=1		
		END				
	END		
		...
		SET @sCodRuolo=(SELECT TOP 1 Valore FROM T_Config WHERE ID=681)
				...
				IF @bErrore=0
	BEGIN	
		SET @uIDEpisodioInizio=(SELECT TOP 1 IDEpisodio	
								FROM T_MovEvidenzaClinica
		...
		BEGIN			
			...
			SET @xTimeStamp.modify('insert <IDEpisodio>{sql:variable("@uIDEpisodioInizio")}</IDEpisodio> into (/TimeStamp)[1]')
						...
						SET @sInfoTimeStamp='Azione: BO_SpostaEvidenzaClinica'
