CREATE PROCEDURE [dbo].[MSP_AggMovCartelleInVisione](@xParametri AS XML)
AS
BEGIN
	...
	DECLARE @xParLog AS XML
		...
		SET @sSQL='UPDATE T_MovCartelleInVisione ' + CHAR(13) + CHAR(10) +
			  'SET '
	...
	SET @sCodRuolo=ISNULL(@sCodRuolo,'')	
		...
		IF @xParametri.exist('/Parametri/IDCartellaInVisione')=1
		BEGIN
			...
			IF 	ISNULL(@sGUID,'') <> '' 
					SET @uIDCartellaInVisione=CONVERT(UNIQUEIDENTIFIER,	@sGUID)	
					END	
	...
	END	
		...
		IF @xParametri.exist('/Parametri/CodStatoCartellaInVisione')=1
	BEGIN
		...
		IF @sCodStatoCartellaInVisione <> ''
				SET	@sSET= @sSET + ',CodStatoCartellaInVisione=''' + @sCodStatoCartellaInVisione +''''	+ CHAR(13) + CHAR(10)				
			ELSE
				SET	@sSET= @sSET + ',CodStatoCartellaInVisione=NULL'	+ CHAR(13) + CHAR(10)		
	END		
	...
	IF @sSET <> ''	
		BEGIN
						...
						IF @uIDCartellaInVisione IS NULL
				SET @sWHERE='WHERE 1=0'
			ELSE	
				SET @sWHERE =' WHERE ID=''' + convert(varchar(50),@uIDCartellaInVisione) +''''
			...
			SET @xTimeStamp.modify('insert <IDEntita>{sql:variable("@uIDCartellaInVisione")}</IDEntita> into (/TimeStamp)[1]')
	
						SET @xTimeStamp.modify('delete (/TimeStamp/CodEntita)[1]') 						
			SET @xTimeStamp.modify('insert <CodEntita>CIV</CodEntita> into (/TimeStamp)[1]')
			...
			SET @xTemp=
				(SELECT * FROM 
					(SELECT  *
					 FROM T_MovCartelleInVisione
					 WHERE ID=@uIDCartellaInVisione									) AS [Table]
