CREATE PROCEDURE [dbo].[MSP_BO_SpostaGruppoSchedaPaz]
		(@xParametri XML, 
		 @bErrore BIT OUTPUT		
		 )  
AS
BEGIN
	...
	BEGIN
		...
		INSERT INTO @tblErrori
				VALUES ('(GSCHPaz001) IDPaziente NON specificato')
	END
	ELSE
	BEGIN
		...
		SET @uIDEntitaFine=@uIDPazienteFine
					  ...
					  WHERE IDEntita=@uIDPazienteInizio AND							
							CodEntita='PAZ'  
																			 )					
				IF @nRecord =0
		BEGIN
			...
			INSERT INTO @tblErrori
				VALUES ('(GSCHPaz002) ERRORE Nessuna Scheda di tipo PAZ da aggiornare')
		END
		ELSE
		BEGIN	
			...
			BEGIN
								SET @bErrore=1
					INSERT INTO @tblErrori
						VALUES('(GSCHPaz0032) ERRORE Stai cercando di spostare schede PAZ sullo stesso paziente')	
			END
			...
			BEGIN
				...
				WHERE 
					IDEntita=@uIDPazienteInizio AND							
					CodEntita='PAZ'  
					...
					BEGIN								
						...
						SET @xTimeStampSingolo=CONVERT(XML,
										...
										'</Parametri>')						
					    						...
					    						INSERT INTO @tblErrori 
							EXEC MSP_BO_SpostaSchedaPaz @xTimeStampSingolo, @bErroreSingolo  OUTPUT
