CREATE PROCEDURE [dbo].[MSP_SelStampaSchedaPercorsoModello1](@xParametri XML)
AS
...
BEGIN
	...
	SELECT TOP 1 
			...
			@sCodUAScheda= CodUA,
			@uIDCartellaAmbulatoriale = IDCartellaAmbulatoriale
	...
	IF @sCodEntitaScheda='PAZ'
		BEGIN
			SELECT 
				...
				@sDescrizioneStatoCartella = SC.Descrizione
			FROM T_MovCartelleAmbulatoriali C
						LEFT JOIN T_StatoCartella SC
							ON C.CodStatoCartella=SC.Codice
			WHERE
				C.ID =  @uIDCartellaAmbulatoriale
