CREATE PROCEDURE [dbo].[MSP_BO_SpostaGruppoSchedaEpi]
		(@xParametri XML, 
		 @bErrore BIT OUTPUT		
		 )  
AS
BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GGG001) ERRORE : nessuna cartella di origine trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN
	...
	BEGIN
		...
		IF @nQta =0 
			BEGIN			
				INSERT INTO @tblErrori
						VALUES('(GSCHEpiO001) ERRORE : nessuna cartella di destinazione trovata')
				SET @bErrore=1
			END
		ELSE
			BEGIN
				IF @nQta>1 
					BEGIN				
						INSERT INTO @tblErrori
							VALUES('(GSCHEpiO002) ERRORE : ho trovato più di una cartella di destinazione, n°: ' +CONVERT(VARCHAR(255),@nQta))									
						SET @bErrore=1
					END
				ELSE
					BEGIN			
	...
	BEGIN
		SET @bErrore=1
		INSERT INTO @tblErrori
				VALUES ('(GSCHEpiO003) IDEpisodio NON trovato')
	END
	ELSE
	BEGIN								
				SET @nRecord=(
					...
					FROM T_MovSchede 
					WHERE 
						IDEntita=@uIDEpisodioInizio AND
		...
		BEGIN
			...
			INSERT INTO @tblErrori
				VALUES ('(GSCHEpiO004) ERRORE Nessuna Scheda EPI da aggiornare per la cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')
		END
		ELSE
		BEGIN	
			...
			BEGIN
								SET @bErrore=1
					INSERT INTO @tblErrori
						VALUES('(GAPPRO005) ERRORE Stai cercando di spostare Schede EPI sulla stessa cartella {' + CONVERT(VARCHAR(50),@uIDCartellaInizio) +'}')	
			END
			...
			BEGIN
				...
				FROM T_MovSchede
				WHERE
						 IDEntita=@uIDEpisodioInizio AND									 IDEpisodio=@uIDEpisodioInizio AND
						 CodEntita='EPI' AND
						 Storicizzata = 0																												
				BEGIN TRANSACTION											
					...
					BEGIN								
						...
						SET @xTimeStampSingolo.modify('insert <IDEntitaInizio>{sql:variable("@uIDScheda")}</IDEntitaInizio> into (/)[1]')


						SET @xTimeStampSingolo.modify('delete (/IDEntitaFine)[1]') 			
						SET @xTimeStampSingolo.modify('insert <IDEntitaFine>{sql:variable("@uIDEpisodioFine")}</IDEntitaFine> into (/)[1]')
						...
						INSERT INTO @tblErrori 
							EXEC MSP_BO_SpostaSchedaEpi @xTimeStampSingolo, @bErroreSingolo  OUTPUT
